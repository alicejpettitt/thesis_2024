# Taken from https://github.com/fpesceKU/BLOCKING/tree/main
...
def blocker(array, multi=1):
    ...
    for n in n_blocks_try:
        if n <= 0:
            continue  # Skip values of n that are less than or equal to zero
        bs = dimension / n
        if bs < 6: ## ADDED BY ALICE - 30TH OCT 2023
            continue  # Skip this block if bs is less than 6 
        if (dimension % n == 0) and (rep % bs == 0):
